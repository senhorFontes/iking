...
<div class="scroll-pane products_block">
	<div style="width: {sizeof($products)*148+155}px;">
	...
	<tr>
		...
		{foreach from=$products item=product name=for_products}
			...
			<td style="width:148px;" class="ajax_block_product comparison_infos">
				<div class="item {if $smarty.foreach.for_products.first}first_item{elseif $smarty.foreach.for_products.last}last_item{/if}" >
					...
					{if $product->specificPrice AND $product->specificPrice.reduction}
						<p class="reduction_percent"><span>sale</span> <span class="reduction_percent_display">
					...
					{/if}
					<h5><a href="{$product->getLink()}" title="{$product->name|escape:'htmlall':'UTF-8'}">{$product->name|truncate:35:'...'|escape:'htmlall':'UTF-8'}</a></h5>
					<p class="category_name">
						{$product->category}
					</p>
					{if isset($product->available_for_order) && $product->available_for_order && !isset($restricted_country_mode)}{if $product->quantity > 0}<span class="availability">{l s='Available'}</span>{elseif (isset($product->quantity_all_versions) && $product->quantity_all_versions > 0)}{l s='Product available with different options'}{else}<span class="outofstock">{l s='Out of stock'}</span>{/if}{/if}

					<div class="comparison_product_infos">

					{if isset($product->show_price) && $product->show_price && !isset($restricted_country_mode) && !$PS_CATALOG_MODE}						
						{if $product->specificPrice AND $product->specificPrice.reduction}
							<span class="price-discount">{convertPrice price=$product->getPriceWithoutReduct($taxes_behavior)}</span>
						{/if}
					...
					<div class="product_desc"><a href="{$product->getLink()}" title="{l s='More'}">{$product->description_short|strip_tags|truncate:120:'...'}</a></div>
