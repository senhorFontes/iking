...
class BlockAdvertising2 extends Module
{
	...
	public function __construct()
	{
		...
		$this->displayName = $this->l('Block the second advertising on the right column.');
	...
	protected function initialize()
	{
		...
		if ((Shop::getContext() == Shop::CONTEXT_GROUP  || Shop::getContext() == Shop::CONTEXT_SHOP)
			&& file_exists(_PS_MODULE_DIR_.$this->name.'/'.$this->adv_imgname.'-g'.$this->context->shop->getContextShopGroupID().'.'.Configuration::get('BLOCKADVERT_IMG_EXT2')))
			$this->adv_imgname .= '-g'.$this->context->shop->getContextShopGroupID();
		if (Shop::getContext() == Shop::CONTEXT_SHOP
			&& file_exists(_PS_MODULE_DIR_.$this->name.'/'.$this->adv_imgname.'-s'.$this->context->shop->getContextShopID().'.'.Configuration::get('BLOCKADVERT_IMG_EXT2')))
			$this->adv_imgname .= '-s'.$this->context->shop->getContextShopID();

		$this->adv_img = Tools::getMediaServer($this->name)._MODULE_DIR_.$this->name.'/'.$this->adv_imgname.'.'.Configuration::get('BLOCKADVERT_IMG_EXT2');
		$this->adv_link = htmlentities(Configuration::get('BLOCKADVERT_LINK2'), ENT_QUOTES, 'UTF-8');
		$this->adv_title = htmlentities(Configuration::get('BLOCKADVERT_TITLE2'), ENT_QUOTES, 'UTF-8');
	}
	
	public function install()
	{
		Configuration::updateGlobalValue('BLOCKADVERT_LINK2', 'http://www.prestashop.com/');
		Configuration::updateGlobalValue('BLOCKADVERT_TITLE2', 'PrestaShop');
		...
		foreach (scandir(_PS_MODULE_DIR_.$this->name) as $file)
			if (in_array($file, array('advertising.jpg', 'advertising.gif', 'advertising.png')))
				Configuration::updateGlobalValue('BLOCKADVERT_IMG_EXT2', substr($file, strrpos($file, '.') + 1));
	...
	{
		Configuration::deleteByName('BLOCKADVERT_LINK2');
		Configuration::deleteByName('BLOCKADVERT_TITLE2');
		Configuration::deleteByName('BLOCKADVERT_IMG_EXT2');
	...
	private function _deleteCurrentImg()
	{
		...
		if ($this->adv_imgname != 'advertising' && file_exists(_PS_MODULE_DIR_.$this->name.'/'.$this->adv_imgname.'.'.Configuration::get('BLOCKADVERT_IMG_EXT2')))
			unlink(_PS_MODULE_DIR_.$this->name.'/'.$this->adv_imgname.'.'.Configuration::get('BLOCKADVERT_IMG_EXT2'));
		
		// Update the extension to the global value or the shop group value if available
		Configuration::deleteFromContext('BLOCKADVERT_IMG_EXT2');
		Configuration::updateValue('BLOCKADVERT_IMG_EXT2', Configuration::get('BLOCKADVERT_IMG_EXT2'));
	...
	public function postProcess()
	{
		...
		if (Tools::isSubmit('submitAdvConf'))
		{
			if (isset($_FILES['adv_img']) && isset($_FILES['adv_img']['tmp_name']) && !empty($_FILES['adv_img']['tmp_name']))
			{
				...
				{
					Configuration::updateValue('BLOCKADVERT_IMG_EXT2', substr($_FILES['adv_img']['name'], strrpos($_FILES['adv_img']['name'], '.') + 1));
					...
					if (!move_uploaded_file($_FILES['adv_img']['tmp_name'], _PS_MODULE_DIR_.$this->name.'/'.$this->adv_imgname.'.'.Configuration::get('BLOCKADVERT_IMG_EXT2')))
			...
			elseif (Shop::getContext() == Shop::CONTEXT_SHOP || Shop::getContext() == Shop::CONTEXT_GROUP)
				Configuration::deleteFromContext('BLOCKADVERT_LINK2');
			...
			if ($title = Tools::getValue('adv_title'))
				Configuration::updateValue('BLOCKADVERT_TITLE2', $title);
			elseif (Shop::getContext() == Shop::CONTEXT_SHOP || Shop::getContext() == Shop::CONTEXT_GROUP)
				Configuration::deleteFromContext('BLOCKADVERT_TITLE2');
	...
	{
		$this->context->controller->addCSS($this->_path.'blockadvertising2.css', 'all');
	}
}